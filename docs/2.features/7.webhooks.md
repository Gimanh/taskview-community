...
A webhook URL must use `http` or `https` and point to a **public** address. URLs that resolve to loopback (`127.0.0.1`, `localhost`), private (`10/8`, `172.16/12`, `192.168/16`), link-local or cloud-metadata ranges are rejected when you save the webhook, and checked again before every delivery. Redirects are not followed: if your endpoint answers with a `3xx`, the delivery is recorded as failed with that status code.

On a self-hosted instance where your receivers (n8n, CI runners, internal bots) live on the same private network, set `WEBHOOKS_ALLOW_PRIVATE_URLS=true` in the API environment. See [environment variables](/docs/configuration/environment-variables) for details.
...
Run with `node webhook-receiver.js` and set the webhook URL to `http://localhost:4545`. Loopback addresses are refused by default, so for this local test start the API with `WEBHOOKS_ALLOW_PRIVATE_URLS=true`, or expose the receiver through a tunnel (ngrok, Cloudflare Tunnel) and use its public URL.
