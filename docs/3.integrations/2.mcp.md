...
Cloud clients register themselves on your instance before the first authorization (dynamic client registration, RFC 7591). That is **off by default**: set `OAUTH_DYNAMIC_REGISTRATION=true` on the API, or every cloud client fails at the registration step with `403` before the consent screen ever opens.
::
...
- `OAUTH_DYNAMIC_REGISTRATION=true` on the API — lets clients register themselves. Without it the registration endpoint answers `403`, is left out of the discovery metadata, and no new OAuth connection can be started. Turning it off again later does not affect apps that are already connected.
