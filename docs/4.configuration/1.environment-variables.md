...
| `OAUTH_DYNAMIC_REGISTRATION` | No | `false` | Whether AI clients may register themselves as OAuth clients (RFC 7591) when connecting to the [MCP server](/docs/integrations/mcp#oauth-cloud-clients). Off by default: the registration endpoint returns 403 and cloud clients such as ChatGPT or Claude cannot start an OAuth connection. Set to `true` to let them connect. Turning it back off later does not affect apps that are already connected. |
...
| `WEBHOOKS_ALLOW_PRIVATE_URLS` | No | `false` | Outgoing [webhooks](/docs/features/webhooks) may only target public addresses: loopback, private (`10/8`, `172.16/12`, `192.168/16`), link-local and cloud-metadata ranges are refused when a webhook is saved and again on every delivery, and redirects are never followed. Set to `true` on a self-hosted install whose receivers (n8n, CI, internal bots) live on the same private network. Never enable this on a public multi-tenant instance. The value must be `true` or `false`; anything else stops the server at startup. |
...
# Let webhooks target receivers on the private network (self-hosted only)
#WEBHOOKS_ALLOW_PRIVATE_URLS=false
