...
export const canManageSprints = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!permissions) {
        $logger.error('Can not get permissions for canManageSprints middleware');
        return res.status(500).end();
    }

    if (permissions.hasPermissions(GoalPermissions.SPRINT_CAN_MANAGE)) return next();
