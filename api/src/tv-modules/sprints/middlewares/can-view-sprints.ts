...
export const canViewSprints = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!permissions) {
        $logger.error('Can not get permissions for canViewSprints middleware');
        return res.status(500).end();
    }

    if (permissions.hasPermissions(GoalPermissions.SPRINT_CAN_VIEW)) return next();
